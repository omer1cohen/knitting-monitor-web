...
(function (root) {
  ...
  async function buildSnapshotFromSupabase(url, key, fetchFn, opts) {
    ...
    const machines = ms.map(m => ({
      conveyor_id: m.conveyor_id, station: m.station, state: m.state, reason: m.reason,
    ...
    const shifts = Object.keys(byShift).map(k => {
      ...
      let run = 0, stop = 0, gp = 0, gs = 0, gg = 0, stops = 0; const per = {}; const mach = [];
      rows.forEach(b => {
        run += b.run_s; stop += b.stop_s; gp += b.gray_power_s; gs += b.gray_station_s; gg += b.gray_gap_s; stops += b.stop_count;
        const st = stationOf(stations, b.conveyor_id);
        const p = (per[st] ||= [0, 0, 0]); p[0] += b.run_s; p[1] += b.stop_s; p[2] += b.gray_power_s + b.gray_station_s + b.gray_gap_s;
        const av = b.run_s + b.stop_s;
        mach.push({ conveyor_id: b.conveyor_id, station: st, run_s: b.run_s, stop_s: b.stop_s,
          gray_s: b.gray_power_s + b.gray_station_s + b.gray_gap_s, stop_count: b.stop_count,
      ...
      return { shift_date: sd, shift_label: label, shift_name: name, run_s: run, stop_s: stop,
        gray_power_s: gp, gray_station_s: gs, gray_gap_s: gg, stops, util: av > 0 ? run / av : 1,
    ...
    return { generated_at, stations, n_conveyors: ms.length, machines, shifts, events: ev };
  }
